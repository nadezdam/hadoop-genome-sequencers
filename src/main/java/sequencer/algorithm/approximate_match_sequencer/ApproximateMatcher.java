...
class ApproximateMatcher {
	...
	private void findMinEditElement() {
		int i = m - 1;
		this.min_edit_el = Integer.MAX_VALUE;
		for (int k = 0; k < n; k++) {

			if (matrix[i][k] < this.min_edit_el) {
				this.min_edit_el = matrix[i][k];
			}
		}
	}
	...
	private void tracebackStep() {
		int i = m - 1;
		for (int j = 0; j < n; j++) {
			if (matrix[i][j] == this.min_edit_el)
				tracebackStep(i, j, "", "");
		}
	}
	...
	private void tracebackStep(int i, int j, String genomeAlign,
		...
		if (traceback[i][j] == ZERO) {
			...
			System.out.println("Number of edits: " + this.min_edit_el);
			System.out.println();
